...
go(function () use ($rpc) {
    ...
    $client->selectService('ServiceOne')->callAction('a2')->onSuccess(function (\EasySwoole\Rpc\Task $task, \EasySwoole\Rpc\Response $response, ?\EasySwoole\Rpc\ServiceNode $serviceNode) {
        var_dump('success' . $response->getMessage());
    });
    $client->call(1.5);
});